...
- `CONFIG_ERROR`: missing or invalid runtime environment configuration, including a forbidden fixed-site override. The verified base URL and endpoint paths are source-controlled in `src/checkin/site_config.py` and cannot be replaced through the environment.
